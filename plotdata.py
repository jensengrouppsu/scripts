...
def plot_abs(d, args):
    ...
    if 'EXCITATIONS' in d.calctype:
        string = dedent('''\
        from chemPackage.constants import HART2NM, HART2EV, PI
        ...
        from chemPackage.constants import HART2NM
        from numpy import linspace
...
def plot_pol(d, args):
    ...
    if set(['POLARIZABILITY', 'FD']) <= d.calctype or set(['OPTICAL ROTATION', 'FD']) <= d.calctype:
        string = dedent('''\
        from scipy.interpolate import InterpolatedUnivariateSpline
        from chemPackage.constants import HART2EV
...
def plot_ord(d, args):
    ...
    if (d.ord != None or d.gtensor != None) and d.e_frequencies != None:
        string = dedent('''\
        from scipy.interpolate import InterpolatedUnivariateSpline
        from chemPackage.constants import HART2WAVENUM, HART2NM
...
def plot_field(d, args):
    ...
    from chemPackage.drawing import drawField
    efield = array([])
    atomfield = []
    distance = []
    for i in [0, 1, 2]:
        A, B, tmp = drawField(d, {0}, calctype='{1}', freq=0,
                              dir=i, scale=1.2, draw=False)
        if i == 0:
            efield = tmp
        else:
            efield = efield + tmp
